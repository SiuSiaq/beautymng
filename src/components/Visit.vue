  ...
  <v-dialog v-model="dialog" width="800">
    <template v-slot:activator="{ on, attrs }">
      <v-list-item v-bind="attrs" v-on="on">
        <v-list-item-content>
          <v-list-item-title>{{ treatment ? event.clientName : event.name }}</v-list-item-title>
    ...
    <v-card>
      <v-card-title
        >Szczegóły zabiegu<v-spacer></v-spacer
        ...
        <v-row>
          ...
          <v-col cols="12" md="6">
            ...
            <div class="subtitle-1">
              {{
                event.additionalTreatments.length > 0
                  ? additionalTreatments
                  : "Brak dodatkowych zabiegów"
              }}
            </div>
          </v-col>
          <v-col cols="12" md="6">
