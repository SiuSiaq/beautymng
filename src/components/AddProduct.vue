...
export default {
  data: () => ({
    ...
    nameRules: [
      ...
      (v) =>
        (v && v.length <= 50) ||
        "Nazwa produktu musi być krótsza niż 50 znaków",
    ],
    producer: "",
    producerRules: [
      (v) =>
        (v && v.length <= 50) ||
        "Nazwa producenta musi być krótsza niż 50 znaków",
    ],
    unit: "",
  ...
  methods: {
    ...
    async submit() {
      ...
      const newProduct = {
        ...
        color: this.color,
        description: this.productDesc !== undefined ? this.productDesc : "",
        unit: this.unit,
        newAmount: parseFloat(this.newAmount),
        plannedAmount: this.amount ? parseFloat(this.amount) : 0,
        used: 0,
        registered: new Date(),
      };
